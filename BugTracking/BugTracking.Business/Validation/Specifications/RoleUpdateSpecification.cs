...
{
    public class RoleUpdateSpecification : AbstractValidator<BusinessObjects.AspNetRole>
    {
        public RoleUpdateSpecification()
        {
            RuleFor(x => x.Name)
        ...
        public bool BeUniqueName(string Name, string Id)
        {
            using (var db = new BugTrackingEntities())
            {

                int count = db.AspNetRoles.Where(x => x.Name == Name && x.Active==true && x.Id != Id).Count();
                return (count == 0);
            }
        }
    }
